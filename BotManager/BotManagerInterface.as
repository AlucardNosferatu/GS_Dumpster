...
namespace BotManager
{
	...
	abstract class BaseBot
	{
		...
		*/
		...
		void RunPlayerMove() final
		{
			...
			g_EngineFuncs.RunPlayerMove(m_pPlayer.edict(), m_pPlayer.pev.angles, m_fForwardMove, m_fSideMove, m_fUpMove, m_pPlayer.pev.button, m_pPlayer.pev.impulse, uint8(m_iMSecInterval));
		}
	}
	...
	final class BotManager
	{
		...
		HookReturnCode ClientSay(SayParameters @param)
		{
			...
			{
				SayMessage(ListenPlayer(), "Speak to all bots");
				for (uint uiIndex = 0; uiIndex < m_Bots.length(); ++uiIndex)
				{
					BaseBot @bot = m_Bots[uiIndex];
					if (bot !is null)
					{
						bot.ClientSay(talker, args);
					}
